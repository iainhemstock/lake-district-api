Feature: GET /fells?offset=1 - EXTENDED POSITIVE TESTS WITH OPTIONAL PARAMETERS
	...
	Scenario Outline: API provides contextual navigational links depending on the current page
		...
		And the next href will contain an offset <next offset>
		Examples:
			| prev offset | self offset | next offset |
			| offset=0    | offset=1    | offset=2   |

	Scenario: API provides a simplified projection of a fell
		When making a GET request to http://localhost:8080/api/v1/fells?offset=1
		Then the body will contain a fell name Helvellyn
		And fell self href http://localhost:8080/api/v1/fells/NY342151

	Scenario: API provides pagination metadata for this collection of resources
		When making a GET request to http://localhost:8080/api/v1/fells?offset=1
		Then the body will contain the offset 1
		And the limit 1
