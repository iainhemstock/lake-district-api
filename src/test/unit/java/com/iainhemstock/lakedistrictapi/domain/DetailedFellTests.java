...
public class DetailedFellTests {
    ...
    public void get_fell_name() {
        DetailedFell detailedFell = new DetailedFell(helvellyn, meterToFeetConversionService, latLongToDmsConverter, endpointGeneratorService);
    ...
    @Test
    public void get_self_link() {
        Link expectedLink = new Link(String.format("http://localhost:8080/api/v1/fells/%s", helvellyn.getOsMapRef()));
        Mockito.when(endpointGeneratorService.generateForResourceWithId("fells", helvellyn.getOsMapRef()))
            .thenReturn(expectedLink);
        DetailedFell detailedFell = new DetailedFell(helvellyn, meterToFeetConversionService, latLongToDmsConverter, endpointGeneratorService);
        assertThat(detailedFell.getSelfLink(),
            is(expectedLink));
    }

    @Test
    public void get_parent_link() {
        Link expectedLink = new Link(String.format("http://localhost:8080/api/v1/fells/%s", helvellyn.getParentPeak().getOsMapRef()));
        Mockito.when(endpointGeneratorService.generateForResourceWithId("fells", helvellyn.getParentPeak().getOsMapRef()))
            .thenReturn(expectedLink);
        DetailedFell detailedFell = new DetailedFell(helvellyn, meterToFeetConversionService, latLongToDmsConverter, endpointGeneratorService);
        assertThat(detailedFell.getParentLink(),
            is(expectedLink));
    }
}
